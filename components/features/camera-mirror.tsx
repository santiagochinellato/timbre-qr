...
const CameraMirror = forwardRef<CameraHandle, CameraMirrorProps>(({ onCapture, fullscreen }, ref) => {
  ...
  if (!stream) {
    return (
      <div className={`relative w-full h-full bg-neutral-800 animate-pulse overflow-hidden ${!fullscreen && 'rounded-2xl aspect-[3/4]'}`}>
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden bg-black ${!fullscreen && 'rounded-2xl shadow-xl'}`}>
      <video
        ref={videoRef}
        autoPlay
        playsInline
      ...
      {!fullscreen && (
        <div className="absolute bottom-6 left-0 right-0 flex justify-center z-10">
           <Button onClick={internalCapture} size="lg" className="rounded-full px-8 shadow-lg bg-white text-black hover:bg-gray-200">
            Tomar Foto
           </Button>
        </div>
      )}
